...
class _SettingPageState extends State<SettingPage>
  ...
  void initState() {
    ...
    LocationService.getCountries().then((value) {
      countries = value;
      if (countries.any((element) => element.id == locator<UserProvider>().profile?.countryId)) {
        selectedCountry = countries.firstWhere(
                (element) => element.id == locator<UserProvider>().profile?.countryId);
      }
  ...
  Future<File?> compressImage(XFile file) async {
    ...
    if (result == null) return null; // Handle the null case
