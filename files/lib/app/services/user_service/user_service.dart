...
class UserService{
  ...
  static Future<bool> logout() async {
    try {
      ...
      // Check if the response contains the 'success' key and ensure it's a boolean
      if (jsonResponse['success'] != null && jsonResponse['success'] is bool) {
        bool success = jsonResponse['success'];
        if (success) {
          // If logout is successful, clear any locally stored user data (SecureStorage and SharedPreferences)
          await clearUserData();
          return true;
        }
      }

      // If the 'success' field is missing or not a boolean, or if it's false, return false
      return false;
    } catch (e) {
