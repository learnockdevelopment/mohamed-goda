...
class UserProfileWidget {
  static Widget profileItem(
      String name, String count, String iconPath, Color color,
      {int width = 24}) {
    return Column(
      children: [
        Container(
    ...
    return SingleChildScrollView(
      ...
      child: (profile.about?.isEmpty ?? true)
          ? Padding(
              ...
              child: emptyState(AppAssets.bioEmptyStateSvg, appText.noBiography,
                  appText.noBiographyDesc),
            )
          : Column(
  ...
  static Widget classesPage(ProfileModel profile) {
    return (profile.webinars?.isEmpty ?? true)
        ? SingleChildScrollView(
            ...
            child: Padding(
              padding: const EdgeInsets.only(top: 30),
              child: emptyState(AppAssets.courseEmptyStateSvg,
                  appText.noCourses, appText.noCoursesDesc),
            ),
          )
        : GridView(
            shrinkWrap: true,
            ...
            children: List.generate(profile.webinars?.length ?? 0, (index) {
              return courseItem(profile.webinars![index],
    ...
    return (profile.badges?.isEmpty ?? true)
        ? SingleChildScrollView(
            ...
            child: Padding(
              ...
              child: emptyState(AppAssets.badgesEmptyStateSvg, appText.noBadges,
  ...
  static Widget instructorPage(ProfileModel profile) {
    return (profile.organizationTeachers?.isEmpty ?? true)
        ? SingleChildScrollView(
            ...
            child: Padding(
              padding: const EdgeInsets.only(top: 20),
              child: emptyState(AppAssets.providersEmptyStateSvg,
                  appText.noInstructor, appText.noInstructorProfileDesc),
            ),
          )
        : GridView.builder(
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            padding: const EdgeInsets.only(
                right: 21, left: 21, bottom: 100, top: 80),
            ...
            itemBuilder: (context, index) {
              return userProfileCard(profile.organizationTeachers![index], () {
                nextRoute(UserProfilePage.pageName,
                    arguments: profile.organizationTeachers?[index].id);
              });
            },
          );
  }

  static Widget tabView(String title1, String title2, bool isOnTitle1,
                ...
                child: GestureDetector(
                  ...
                  child: Center(
                    child: Text(
                      title1,
                      style: style14Regular()
                          .copyWith(color: isOnTitle1 ? Colors.white : greyA5),
                    ),
                  ),
                ),
              ),
